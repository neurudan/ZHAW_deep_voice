...
from .objectives_clustering import create_loss_functions
...
def create_and_train(num_epochs=1000, batch_size=100, epoch_batches=10, network_params_file_in=None,
    ...
    margin = config.getfloat('pairwise_cosface', 'margin')
    scale = config.getint('pairwise_cosface', 'scale')
    ...
    train_fn, val_fn = create_loss_functions(input_var, network, target_var, margin, scale)
...
def train(X, y, num_epochs, train_fn, val_fn=None, train_iterator=None, validation_iterator=None):
    ...
    theano.printing.pydotprint(train_fn, outfile='/data/test.svg', format='svg')
