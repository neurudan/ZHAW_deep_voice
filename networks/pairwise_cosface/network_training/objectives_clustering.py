...
def create_loss_functions(input_var, network, target_var, margin, scale):
    ...
    train_updates = lasagne.updates.adadelta(train_loss,
                                             train_params,
    ...
    train_fn = theano.function([input_var, target_var], train_loss, updates=train_updates)
    val_fn = theano.function([input_var, target_var], [test_loss, test_class_predict])
    return train_fn, val_fn

def cosface_loss(logits, labels, margin, scale):
    logits_m = logits - margin
    inv_labels = 1 - labels
    targets = scale * (logits * inv_labels + logits_m * labels)
    #softmax_logits = lasagne.nonlinearities.softmax(targets)
    loss = lasagne.objectives.categorical_crossentropy(targets, labels)
    return loss.mean()

def cosface_logits(logits, label, margin, scale):
    target_logits = logits - margin
    logits = logits * (1 - label) + target_logits * label
    logits = logits * scale
    return logits
